...
@router.get("/maintenance/bakim-kayitlari", response_model=list[MaintenanceResponse])
def read_maintenance_records(
    db: DBSession,
    _: TechUser,
) -> list[MaintenanceResponse]:
    return [_build_maintenance_response(bakim) for bakim in list_maintenance_records(db)]
...
def _build_maintenance_response(bakim) -> MaintenanceResponse:
    return MaintenanceResponse(
        ...
        oncelik=bakim.oncelik,
        maliyet_tl=bakim.maliyet_tl,
        parca_maliyeti_tl=bakim.parca_maliyeti_tl,
        iscilik_maliyeti_tl=bakim.iscilik_maliyeti_tl,
        tedarikci=bakim.tedarikci,
        kilometre=bakim.kilometre,
        planlanan_tarih=bakim.planlanan_tarih,
        durum=bakim.durum,
