...
from backend.app.schemas.fleet import (
    ...
    VehicleListResponse,
    VehicleResponse,
    VehicleScrapSaleRequest,
    VehicleScrapSaleResponse,
    VehicleUpdate,
)
...
FleetReadUser = Annotated[
    Personel,
    Depends(require_roles("Sistem Yoneticisi", "Bakim Teknisyeni", "Geri Donusum Operatoru")),
]
...
@router.post("/araclar/{arac_id}/hurda-satis", response_model=VehicleScrapSaleResponse)
def scrap_vehicle_sale_endpoint(
    arac_id: int,
    payload: VehicleScrapSaleRequest,
    db: DBSession,
    current_user: AdminUser,
) -> VehicleScrapSaleResponse:
    arac = get_vehicle_or_404(db, arac_id)
    arac, gelir = scrap_and_sell_vehicle(db, arac, payload)
    log_action(
        db,
        actor=current_user,
        islem_tipi="AracHurdaSatis",
        aciklama=f"{arac.plaka} plakali arac hurda satisa ayrildi.",
        varlik_tipi="Arac",
        varlik_id=arac.id,
    )
    db.commit()
    return VehicleScrapSaleResponse(
        arac=VehicleResponse.model_validate(arac),
        gelir_kaydi_id=gelir.id,
        mesaj="Arac hurdaya ayrildi ve muhasebeye bekleyen gelir kaydi olustu.",
    )
