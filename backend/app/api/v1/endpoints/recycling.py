...
@router.get("/teslimler", response_model=list[DeliveryResponse])
def read_deliveries(
    db: DBSession,
    _: OperatorUser,
) -> list[DeliveryResponse]:
    return [DeliveryResponse.model_validate(teslim) for teslim in list_deliveries(db)]
...
@router.post("/stoklar/manuel", response_model=StockMovementResponse, status_code=status.HTTP_201_CREATED)
def create_manual_stock_endpoint(
    payload: ManualStockCreate,
    db: DBSession,
    current_user: OperatorUser,
) -> StockMovementResponse:
    hareket = create_manual_stock_movement(db, payload, current_user)
    log_action(
        db,
        actor=current_user,
        islem_tipi="ManuelStok",
        aciklama=f"{payload.atik_tipi.value} icin {payload.miktar_kg} kg manuel stok eklendi.",
        varlik_tipi="StokHareketi",
        varlik_id=hareket.id,
    )
    db.commit()
    return StockMovementResponse.model_validate(hareket)


@router.get("/stok-hareketleri", response_model=list[StockMovementResponse])
def read_stock_movements(
    db: DBSession,
    _: OperatorUser,
) -> list[StockMovementResponse]:
    return [StockMovementResponse.model_validate(hareket) for hareket in list_stock_movements(db)]
...
@router.get("/satislar", response_model=list[SaleResponse])
def read_sales(
    db: DBSession,
    _: OperatorUser,
) -> list[SaleResponse]:
    return [_build_sale_response(satis) for satis in list_sales(db)]
...
def _build_sale_response(satis) -> SaleResponse:
    return SaleResponse(
        ...
        alici_firma=satis.alici_firma,
        belge_no=satis.belge_no,
        durum=satis.durum,
