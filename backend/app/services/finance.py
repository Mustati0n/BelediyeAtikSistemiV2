...
def create_salary_payment(db: Session, payload: SalaryPaymentCreate) -> MaasOdeme:
    ...
    db.add(payment)
    db.flush()
    _create_salary_expense(db, payment)
    db.refresh(payment)
    return payment
...
def create_batch_salary_payments(db: Session, payload: BatchSalaryCreate) -> list[MaasOdeme]:
    ...
    for odeme in odemeler:
        _create_salary_expense(db, odeme)
    return odemeler
...
def _create_salary_expense(db: Session, payment: MaasOdeme) -> GiderKaydi:
    expense = GiderKaydi(
        tarih=datetime.combine(payment.odeme_tarihi, time.min, tzinfo=timezone.utc),
        tutar=payment.tutar,
        aciklama=(
            f"Maas odemesi: {payment.personel.ad_soyad} "
            f"{payment.donem_ay:02d}/{payment.donem_yil} {payment.odeme_tipi.value}"
        ),
