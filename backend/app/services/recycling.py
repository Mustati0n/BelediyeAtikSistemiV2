...
from backend.app.schemas.recycling import DeliveryCreate, ManualStockCreate, SaleCreate, SortingCreate
...
def list_deliveries(db: Session) -> list[TesisTeslim]:
    stmt = (
        select(TesisTeslim)
        .options(
            joinedload(TesisTeslim.teslim_eden_sofor),
        ...
        .order_by(TesisTeslim.tarih.desc(), TesisTeslim.id.desc())
    )
...
def approve_delivery(db: Session, teslim: TesisTeslim, operator: Personel) -> TesisTeslim:
    ...
    teslim.onay_tarihi = datetime.now(timezone.utc)
    teslim.teslim_alan_operator = operator
    db.flush()
    if teslim.atik_tipi is not None and not teslim.stok_hareketleri:
        _create_stock_movement(
            db,
            teslim=teslim,
            waste_type=teslim.atik_tipi,
            amount=teslim.toplam_kg,
            note=f"Teslim onayi ile otomatik stok girisi: teslim #{teslim.id}",
        )
    db.refresh(teslim)
    return teslim
...
def list_stock_movements(db: Session) -> list[StokHareketi]:
    stmt = select(StokHareketi).order_by(StokHareketi.tarih.desc(), StokHareketi.id.desc())
    return list(db.scalars(stmt))


def create_manual_stock_movement(
    db: Session,
    payload: ManualStockCreate,
    operator: Personel,
) -> StokHareketi:
    if payload.miktar_kg <= 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Manuel stok icin pozitif kg girilmelidir.",
        )
    teslim = TesisTeslim(
        tarih=datetime.now(timezone.utc),
        toplam_kg=payload.miktar_kg,
        atik_tipi=payload.atik_tipi,
        aciklama=payload.aciklama or "Manuel stok girisi",
        onaylandi_mi=True,
        onay_tarihi=datetime.now(timezone.utc),
        teslim_eden_sofor=operator,
    ...
    movement = _create_stock_movement(
        db,
        teslim=teslim,
        waste_type=payload.atik_tipi,
        amount=payload.miktar_kg,
        note=payload.aciklama or "Operator manuel stok girisi",
    )
    db.refresh(movement)
    return movement
...
def create_sale(db: Session, payload: SaleCreate) -> Satis:
    ...
    satis = Satis(
        ...
        alici_firma=payload.alici_firma,
...
def list_sales(db: Session) -> list[Satis]:
    stmt = (
        select(Satis)
        .options(joinedload(Satis.stok), joinedload(Satis.gelir_kaydi))
        .order_by(Satis.tarih.desc(), Satis.id.desc())
    )
    return list(db.scalars(stmt))
...
def _create_stock_movement(
    ...
    *,
    ...
    stok = _get_or_create_stock(db, waste_type)
    stok.toplam_miktar_kg += amount
    movement = StokHareketi(
        tarih=datetime.now(timezone.utc),
