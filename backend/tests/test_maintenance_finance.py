...
def calculate_seeded_salary(person) -> Decimal:
    return person.taban_maas + (Decimal(person.cocuk_sayisi) * Decimal("1000.00"))
...
async def test_maintenance_creation_generates_pending_expense(
    ...
    async with AsyncClient(
        ...
        bakim_response = await client.post(
            ...
            json={
                ...
                "bakim_turu": "Motor",
                "oncelik": "Kritik",
                "parca_maliyeti_tl": "9000.00",
                "iscilik_maliyeti_tl": "3500.00",
    ...
    assert payload["bakim_turu"] == "Motor"
    assert payload["oncelik"] == "Kritik"
    assert payload["tedarikci"] == "Gaziantep Yetkili Servis"
    assert payload["kilometre"] == 182000
    assert payload["parca_maliyeti_tl"] == "9000.00"
    assert payload["iscilik_maliyeti_tl"] == "3500.00"
...
async def test_salary_calculation_single_and_batch_payment(
    ...
    async with AsyncClient(
        ...
        single_response = await client.post(
            ...
            json={
                ...
                "tutar": "2500.00",
                "aciklama": "Mesai: Gece vardiyasi (+2500.00 TL)",
    ...
    assert single_response.json()["aciklama"] == "Mesai: Gece vardiyasi (+2500.00 TL)"
    assert batch_response.status_code == 200
    assert batch_response.json()["kayit_sayisi"] == 5

    odemeler = list(db_session.scalars(select(MaasOdeme)))
    assert len(odemeler) == 6
    salary_expenses = list(
        db_session.scalars(
            select(GiderKaydi).where(
                GiderKaydi.durum == OnayDurumu.ONAYLANDI,
                GiderKaydi.aciklama.like("Maas odemesi:%"),
            )
        )
    )
    assert len(salary_expenses) == 6
    expected_salary_expense = Decimal("2500.00") + sum(
        calculate_seeded_salary(person) for person in seeded_users.values() if person.aktif_mi
    )
    assert sum((expense.tutar for expense in salary_expenses), Decimal("0.00")) == expected_salary_expense
