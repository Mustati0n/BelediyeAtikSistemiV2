...
async def test_sale_creates_pending_revenue_and_reduces_stock(
    ...
    async with AsyncClient(
        ...
        sale_response = await client.post(
            ...
            json={
                ...
                "miktar_kg": "30.000",
                "birim_fiyat": "12.50",
                "alici_firma": "Demo Geri Donusum A.S.",
                "belge_no": "FTR-2026-001",
            },
            headers={"Authorization": f"Bearer {operator_token}"},
        )
        ...
        movements_response = await client.get(
            "/api/v1/recycling/stok-hareketleri",
            headers={"Authorization": f"Bearer {operator_token}"},
        )
    ...
    assert payload["alici_firma"] == "Demo Geri Donusum A.S."
    assert payload["belge_no"] == "FTR-2026-001"
    assert "tarih" in payload
    assert payload["durum"] == OnayDurumu.BEKLEMEDE.value
    assert sales_response.status_code == 200
    assert len(sales_response.json()) == 1
    assert sales_response.json()[0]["alici_firma"] == "Demo Geri Donusum A.S."
